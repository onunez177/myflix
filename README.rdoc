...
by the instructors at Tealeaf Academy (http://www.goteleaf.com) I developed the entire 
...
- Capybara: used in conjuction with RSpec to create high-level tests for key
business features.
- Selenium Webkit: used to create feature tests that require javascript.
- VCR: used to record http responses when interacting with the Stripe API
in our tests, to help speed up the test suite.
...
- Users must enter credit card information upon registration and can only
access the site upon successfully paying an initial subscription fee.
...
- Upon failed payment, users will receive an email with a link to re-activate
their account and update credit card information.
...
- If any user ever forgets their password, they can request a password
reset email. Each email routes to a unique url via a uniquely generated
token on the user model.
- Administrators can add videos to the site and also view the 10 most
...
- User can add a video to their queue while watching a movie without interruption of
video playback.
